...
from typing import Callable, NotRequired, TypedDict, TYPE_CHECKING, Any, TypeAlias
...
def parse_response_for_stop_token(
    ...
    if emt_count == 0:
        ...
        print(f"Response is not a valid assistant response: {str_response}")
...
class Qwen3Renderer(Renderer):
    ...
    def _parse_tool_call(self, tool_call_str: str) -> ToolCall | None:
        try:
    ...
    def parse_response(self, response: list[int]) -> tuple[Message, bool]:
        ...
        if "<tool_call>" in assistant_message["content"]:
            # Separate content before tool calls from the tool calls themselves
            parts = assistant_message["content"].split("<tool_call>", 1)

            # Set the assistant content to be the first "block" before the
            # actual tool calls:
            # "<assistant_content><tool_calls>...</tool_call><tool_call>...</tool_call>"
            # If tool parsing fails then we revert this.
            assistant_message_content = parts[0].strip()

            # Now parse each tool call
            all_tool_calls = []

            # Find each tool call match
